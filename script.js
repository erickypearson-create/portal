...
function bindEvents() {
  ...
  classRoom.addEventListener('change', updateRoomAvailabilityMessage);
  document.getElementById('startTime').addEventListener('input', updateRoomAvailabilityMessage);
  document.getElementById('endTime').addEventListener('input', updateRoomAvailabilityMessage);
  weekdayGrid.addEventListener('change', updateRoomAvailabilityMessage);
}
...
function updateRoomAvailabilityMessage() {
  const selectedWeekdays = getSelectedWeekdays();
  const payload = {
    roomId: classRoom.value,
    weekdays: selectedWeekdays,
  ...
  if (conflict) {
    ...
    classConflict.classList.remove('hidden');
    classConflict.classList.remove('conflict-message--error');
